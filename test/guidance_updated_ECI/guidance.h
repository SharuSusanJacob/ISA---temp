...
#define MAX_DELTA_ACC 1.0 // Maximum change in acceleration per cycle (m/s^2)

/* Output structure for guidance algorithm */
typedef struct
{
    double t_go;      // Time to go (s)
    double r;         // Distance to target (m)
    double theta_deg; // Pitch angle (degrees)
    double psi_deg;   // Yaw angle (degrees)
    Vector3 A_M_body; // Body frame acceleration command (m/s^2)
} GuidanceOutput;
