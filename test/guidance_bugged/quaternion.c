...
QuaternionError_t quaternion_create(Quaternion_t* result, double w, double x, double y, double z)
{
...
QuaternionError_t quaternion_from_axis_angle(Quaternion_t* result, const Vector3_t* axis, double angle_rad)
{
    // Parameter validation
...
QuaternionError_t quaternion_to_rotation_matrix(double* matrix, const Quaternion_t* q)
{
    // Parameter validation
    ...
    matrix[1] = 2.0*(x*y - w*z);       // (1,2)
    matrix[2] = 2.0*(x*z + w*y);       // (1,3)
    
    // Row 2
    matrix[3] = 2.0*(x*y + w*z);       // (2,1)
    matrix[4] = w*w - x*x + y*y - z*z;  // (2,2)
    matrix[5] = 2.0*(y*z - w*x);       // (2,3)
    
    // Row 3
    matrix[6] = 2.0*(x*z - w*y);       // (3,1)
    matrix[7] = 2.0*(y*z + w*x);       // (3,2)
